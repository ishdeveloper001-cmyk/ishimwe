...
const DoctorViewModal = ({ open, onClose, doctor }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
      <DialogTitle sx={{ pb: 0 }}>Doctor Profile</DialogTitle>
      <DialogContent dividers>
        <Box className="doctor-profile">
          <Box className="doctor-profile-header">
            <Box className="doctor-profile-image">
              {doctor.profileImage ? <img src={doctor.profileImage} alt={doctor.name} /> : <Avatar sx={{ width: '100%', height: '100%', fontSize: '3rem', bgcolor: 'primary.main' }}>{doctor.name?.charAt(0)}</Avatar>}
            </Box>
            <Box className="doctor-profile-info">
              <Typography variant="h4" component="h2">{doctor.name}</Typography>
              <Typography variant="subtitle1" className="specialization">{doctor.specialization}</Typography>
              ...
              <Chip label={doctor.status === 'available' ? 'Available' : 'Busy'} color={doctor.status === 'available' ? 'success' : 'warning'} size="small" />
            </Box>
          </Box>
          ...
          <Divider />
          <Box className="doctor-profile-details">
            <Typography variant="h6" sx={{ mb: 2 }}><School color="primary" sx={{ mr: 1, verticalAlign: 'middle' }} />Education</Typography>
            <Typography variant="body2" color="text.secondary">{doctor.education}</Typography>
          </Box>
          {doctor.workHistory && <><Divider /><Box className="doctor-profile-details"><Typography variant="h6" sx={{ mb: 2 }}><Work color="primary" sx={{ mr: 1, verticalAlign: 'middle' }} />Work History</Typography><Typography variant="body2" color="text.secondary">{doctor.workHistory}</Typography></Box></>}
          <Divider />
          <Box className="doctor-profile-details">
            <Typography variant="h6" sx={{ mb: 2 }}><AccessTime color="primary" sx={{ mr: 1, verticalAlign: 'middle' }} />Availability</Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>{doctor.availability?.map((day) => <Chip key={day} label={day} size="small" variant="outlined" />)}</Box>
            <Typography variant="body2" color="text.secondary">Working Hours: {doctor.workingHours?.start} - {doctor.workingHours?.end}</Typography>
          </Box>
          <Divider />
          <Box className="doctor-profile-details">
            <Typography variant="h6" sx={{ mb: 2 }}><CalendarMonth color="primary" sx={{ mr: 1, verticalAlign: 'middle' }} />Upcoming Schedule</Typography>
            {upcomingAppointments.length > 0 ? <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>{upcomingAppointments.map((apt) => <Box key={apt.id} sx={{ p: 2, bgcolor: 'action.hover', borderRadius: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><Box><Typography variant="body2" sx={{ fontWeight: 600 }}>{formatDate(apt.date)}</Typography><Typography variant="caption" color="text.secondary">{apt.time} ({apt.duration} min)</Typography></Box><Chip label={apt.status} size="small" color={apt.status === 'scheduled' ? 'info' : 'default'} /></Box>)}</Box> : <Typography variant="body2" color="text.secondary">No upcoming appointments scheduled</Typography>}
          </Box>
        </Box>
      </DialogContent>
      <DialogActions sx={{ p: 2 }}><Button onClick={onClose} variant="outlined">Close</Button></DialogActions>
    </Dialog>
  );
};
